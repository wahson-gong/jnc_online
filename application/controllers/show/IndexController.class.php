...
class IndexController extends   BaseController
{
    ...
    public function uploadAction(){
        ...
        $rs=$_model->where(['yonghuming'=>$_SESSION['tel']])->find()->one();

        $url=AdminController::shuiyinAction($rs['yanhuidanhao'],$filename,$rs['shi']);
...
//        }
  var_dump($_SESSION);
    ...
    public function upload1Action(){
        ...
        $shi=$wdyh->findBySql("select shi from sl_wdyh WHERE id=$id")[0]['shi'];
//        $src=AdminController::shuiyinAction($danhao,$url,$shi);
...
//           $this->ajaxReturn($data);
        ...
        foreach ($result  as $k => $v)
        {
            ...
            $_data["jiangxiang"]=$wdjp->findBySql("select * from sl_zjmd where tel ='{$tel}' ORDER BY xuni DESC ");
            $_data["cunzai"]=$wdjp->findBySql("select * from sl_zjmd where tel ='{$tel}' AND xuni=2");
...
//        $result=$wdjp->where(['yonghuming'=>$tel])->where(['zhuangtai'=>2])->all();
    ...
    public function checkAction(){
        //        $tel=18582479523;
        $tel=$_SESSION['tel'];
        $_modek=new ModelNew('yhmd');
        $aa=$_modek->findBySql("select *from sl_yhmd WHERE kehudianhua=$tel");
        foreach ($aa as $v){
            if ($v['beizhu']!=''){
                echo '6';die;
            }
        ...
        $model=new ModelNew();
        $num=$model->M('wdyh');
        $result=$num->findBySql("select sum(jifen) from sl_wdyh WHERE yonghuming=$tel ORDER BY yonghuming")[0]['sum(jifen)'];
...
//        $data=['1'=>2,'2'=>3,'去'=>3,'我'=>3,'饿'=>3,'人'=>3,'他'=>3,' 有'=>3,'饿1'=>3];
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G1','领奖状态');
        foreach($data as $k => $v){
            $num=$k+2;
            $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('G'.$num, $v['lingjiangzhuangtai']);
        }
...
//循环读取excel文件,读取一条,插入一条
        ...
        for($j=2;$j<=$highestRow;$j++)
        {
            ...
            $data['beizhu'] = $objPHPExcel->getActiveSheet()->getCell("O".$j)->getValue();//获取B列的值
            ...
            '{$data['sheng']}','{$data['shi']}','{$data['qihao']}','{$data['diyubianhao']}','{$data['quyu']}','{$data['beizhu']}'),";
            if ($j%1000==0){
                $sql = rtrim($sql, ',');
               ...
               ,kehudianhua,gengdanrenxingming,sheng,shi,qihao,diyubianhao,quyu,beizhu)  VALUES ".$sql);
                $sql='';
            }
...
//        $rs=IndexController::sjtsAction($lybh,$shuju['kaijiangquyu']);
        $rs=IndexController::sjtsAction($lybh,$_cf_data_dh,$quyu);
        if ($rs==1) {
            $drjl=$model->M('daorujilu');
        ...
        //数据的调试发的方法
    public static function sjtsAction($lybh,$danhao,$quyu){
        ...
        foreach ($_cf_danhao as $v1){
             ...
             $_data_danhao=$model->findBySql("select * from sl_yhmd WHERE yanhuidanhao='{$v1}' AND quyu='".$quyu."'");
             $model->query("update  sl_yhmd set zhuangtai=1 WHERE yanhuidanhao='{$v1}' AND quyu='".$quyu."'");
