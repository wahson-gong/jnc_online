        ...
        if(count($filedListU6)>0)
        {
            ...
            if(trim($u6)!='')
            {
                ...
                foreach ($filedListU6 as $v)
                {
                    ...
                    $page_paeam_array = $page_paeam_array +  array( "u6" => $u6 );
                }
                $where=$where." 1=2 ";
            }
            ...
            foreach ($filedListU6 as $v)
            {
                if(!empty($_REQUEST[$v['u1']]))
                {
                    $page_paeam_array = $page_paeam_array +  array( $v['u1'] => empty($_REQUEST[$v['u1']])?"":$_REQUEST[$v['u1']] );
                }
...
// 	    die();
	    ...
	    // 使用分页类获取分页信息
	    $page = new Page($total, $pagesize, $current, "index.php", $page_paeam_array);
