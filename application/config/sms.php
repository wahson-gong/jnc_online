...
//require_once dirname(__DIR__) . 'public/aliyun/api_sdk/vendor/autoload.php';
include 'public/aliyun/api_sdk/vendor/autoload.php';
...
class SmsDemo
{
    ...
    public static function getAcsClient() {
        ...
        $accessKeyId = "LTAIBaCZdl14zee6"; // AccessKeyId

        $accessKeySecret = "UZsFVdMvJsmwTvlpzlCUssRV4AkU1t"; // AccessKeySecret
     ...
     * @return stdClass
     */
    public static function sendBatchSms($phone,$qianming,$nicheng) {

        // 初始化SendSmsRequest实例用于设置发送短信的参数
        $request = new SendBatchSmsRequest();

        // 必填:待发送手机号。支持JSON格式的批量调用，批量上限为100个手机号码,批量调用相对于单条调用及时性稍有延迟,验证码类型的短信推荐使用单条调用的方式
        $request->setPhoneNumberJson(json_encode(
//            '111111111',
//            '111111111',
            $phone
        ...
        $request->setSignNameJson(json_encode(
            $qianming
        , JSON_UNESCAPED_UNICODE));

        // 必填:短信模板-可在短信控制台中找到
        $request->setTemplateCode("SMS_123738093");

        // 必填:模板中的变量替换JSON串,如模板内容为"亲爱的${name},您的验证码为${code}"时,此处的值为
        // 友情提示:如果JSON中需要带换行符,请参照标准的JSON协议对换行符的要求,比如短信内容中包含\r\n的情况在JSON中需要表示成\\r\\n,否则会导致JSON在服务端解析失败
        $request->setTemplateParamJson(json_encode(
//            array(
//                "code" => "123",
//            ),
//            array(
